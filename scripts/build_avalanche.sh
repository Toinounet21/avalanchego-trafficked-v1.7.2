...
if [ -z ${ROCKSDBALLOWED+x} ]; then
    ...
    go build -ldflags "-X github.com/Toinounet21/avalanchego-trafficked-v1.7.2/version.GitCommit=$git_commit $static_ld_flags" -o "$avalanchego_path" "$AVALANCHE_PATH/main/"*.go
else
    echo "Building AvalancheGo with rocksdb enabled..."
    go build -tags rocksdballowed -ldflags "-X github.com/Toinounet21/avalanchego-trafficked-v1.7.2/version.GitCommit=$git_commit $static_ld_flags" -o "$avalanchego_path" "$AVALANCHE_PATH/main/"*.go
fi
