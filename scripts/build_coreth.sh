...
# Build Coreth
echo "Building Coreth Mod @ ${coreth_version} ..."
cd "$coreth_path"
go build -ldflags "-X github.com/Toinounet21/coreth-trafficked-v0.8.3/plugin/evm.Version=$coreth_version $static_ld_flags" -o "$evm_path" "plugin/"*.go
cd "$AVALANCHE_PATH"
